...
var loadService = function (router, api, options) {
	...
	//* create one, POST	/
	router.post("/", jsonParser, (req, res) => { api.create(req.body, (...arg) => resRows(res, ...arg)); });

	//* update one,	PUT	/
	router.put("/", jsonParser, (req, res) => { api.update(req.body, (...arg) => resRows(res, ...arg)); });

	//* update one, done,	PUT	/{id}/done		//update `done_at` to current time
	router.put("/:id/done", (req, res) => { api.updateDoneAt(req.params.id, (...arg) => resRows(res, ...arg)); });

	//* delete one,	DELETE	/{id}
	router.delete("/:id", (req, res) => { api.delete(req.params.id, (...arg) => resRows(res, ...arg)); });

	//* read one,	GET		/{id}
	router.get("/:id", (req, res) => { api.read(req.params.id, (...arg) => resRows(res, ...arg)); });

	//* read all,	GET		/
	router.get("/", (req, res) => { api.readAll((...arg) => resRows(res, ...arg)); });

	//* read all, expire,	GET		/expire/YYYY-MM-dd hh:mm:ss
	router.get("/expire/:expire_at", (req, res) => { api.readExpire(req.params.expire_at, (...arg) => resRows(res, ...arg)); });
