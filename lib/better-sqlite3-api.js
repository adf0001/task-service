...
getApi( betterSqlite3, options )
  ...
  options
    ...
    .prepare
      set `true` to prepare the table firstly.
...
var getApi = function (betterSqlite3, options) {
  ...
  if (tableName.match(/[^\w\-]/))
    throw "table name contains abnormal character";
...
WHERE id=@id`
  ...
  // readAll		//read all
  var stateGetAll = betterSqlite3.prepare(`SELECT * from ${tableName};`);
  ...
  var stateGetExpireAll = betterSqlite3.prepare(
    `SELECT * FROM ${tableName} WHERE done_at IS NULL;`
  );

  //expire_at: a utc string, or null for all.
  api.readExpire = (expire_at, cb) => {
    var rows;

    if (expire_at) {
      if (!expire_at?.match(regUtc)) {
        cb?.("format fail, expire_at");
        return;
      }
      rows = stateGetExpire.all({ expire_at });
    }
    else {
      rows = stateGetExpireAll.all();
    }
