...
module.exports = {
  "better-sqlite3-api": function (done) {
    ...
    // create
    util
      .promisify(api.create)({
        title: "title",
        expire_at: "2022-12-31 00:00:00",
      })
      .then((rows) => {
      ...
      requestPromise(
        ...
        .then((ret) => {
          ...
          return requestPromise(
            { path: "/test-tasks/", method: "PUT" },
            ret.rows[0]
          );
        })
        .then((ret) => {
          if (ret?.rows?.[0]?.expire_at !== "2022-12-31 01:02:03")
            throw "update fail";
          console.log("update ok");

          //updateDoneAt
          return requestPromise(
            { path: `/test-tasks/${ret.rows[0].id}/done`, method: "PUT" },
            null
          );
        })
        .then((ret) => {
          ...
          //read one
          return requestPromise(
            { path: `/test-tasks/${ret.rows[0].id}`, method: "GET" },
            null
          );
        })
        .then((ret) => {
        ...
        .then((ret) => {
          if (ret?.rows?.[ret?.rows.length - 1]?.id !== newId)
            throw "read all fail";
          ...
          return requestPromise(
            {
              path:
                `/test-tasks/expire/` +
                encodeURIComponent(`2022-12-31 01:02:03`),
              method: "GET",
            },
            null
          );
        ...
        .then((ret) => {
          ...
          //delete
          return requestPromise(
            { path: `/test-tasks/${newId}`, method: "DELETE" },
            null
          );
        })
        .then((ret) => {
          ...
          //read one deleted
          return requestPromise(
            { path: `/test-tasks/${newId}`, method: "GET" },
            null
          );
        })
        .then((ret) => {
...
if (typeof describe === "function")
  describe("task_service", function () {
    for (var i in module.exports) {
      it(i, module.exports[i]).timeout(5000);
    }
  });
